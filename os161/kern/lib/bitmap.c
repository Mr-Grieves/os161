...
int bitmap_Davids_better_isset(struct bitmap * map, u_int32_t index) {
  
  if (index >= map->nbits) {
    panic("Bitmap index out of bounds");
  }
  
  unsigned char i = map->v[index/8]; //Proper byte
  unsigned char specificBit = (index/8)*8;
  specificBit = index - specificBit;
  unsigned char value;
  unsigned char mask;
