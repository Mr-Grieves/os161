...
exorcise(void)
{
	...
	for (i = 0; i < PROCESS_MAX; i++) {
	  if (pid_hash[i] != NULL && pid_hash[i]->numInterested == 0) {
	    DEBUG(DB_THREADS, "Purging PID: %d. New PID total: %d\n", pid_hash[i]->pid, (numPID - 1));
	    pid_purge(pid_hash[i]->pid);
	  }
	}
	    ...
	    struct thread **ret)
{
	...
	result = pid_allocate(&(newguy->pid_ID));
	if (result != 0) {
		goto fail;
	}
	 ...
	 */
	if (ret != NULL) {	
		//kprintf("pid im about to return: %d\n", newguy->pid_ID);
		*ret = newguy;
	}
