...
common_prog(int nargs, char **args)
{
	...
	int status, options;
	pid_wait(ret->pid_ID, &status, options);
...
/*Allows changing of debug flag */
int cmd_df(int n, char **a) {
  //Need 2 arguments 
  if (n != 3) {
    return 1;
  }
  //Extract option number and choice
  int num = atoi(a[1]), choice = -1;
  
  if (!strcmp(a[2], "on") || !strcmp(a[2], "ON")) {
    choice = 1;
  } else if (!strcmp(a[2], "off") || !strcmp (a[2], "OFF")) {
    choice = 0;
  } else {
    kprintf("Usage: df %d on/off\n", num);
    return 1;
  }
...
menu(char *args)
{
	...
	// CHANGE THIS TO USE ACTUAL SYNCRONISATION!!!!!
	while (1) { 
