...
getppages(unsigned long npages)
{
	...
	if (npages >= cm_numEntries) {
	  kprintf("Not enough pages. Remove this when swapping exists\n");
	  return 0;
	}
	...
	if (count != npages) {
	  kprintf("Couldn't allocate contiguous pages. Remove this when swapping exists\n");
	  return 0;
	}
...
struct as_region*
as_define_region(struct addrspace *as, char* reg, vaddr_t vaddr, size_t sz,
