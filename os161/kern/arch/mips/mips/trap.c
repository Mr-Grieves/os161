...
kill_curthread(u_int32_t epc, unsigned code, u_int32_t vaddr)
{
	 ...
	 */
	thread_exit();
	panic("Should've exited. Oh noes");
}
...
{
	/*char padding[] = "'\0''\0''\0'";
	char *token = (char*)argv;
	int i,j;
	char **str_list = kmalloc(sizeof(char**) * (argc + 1));
	str_list[argc] = NULL;
	char **argvNew = kmalloc(sizeof(char**) * (argc + 1));
	argvNew[argc] = NULL;
	
	int tempstack = stack;
	
	//Better recreate argv in reverse..
	for (j = argc-1; j >= 0; j--) {
	  str_list[j] = token;
	  if (j != 0) {
	    while (*token != '\0') {
	...
	//...add it to the stack? Hell if I know
	for (i = 0; i < argc; i++) {
	  int length = strlen(str_list[i]) + 1;
	  stack -= length + (length)%4;
	  copyout(str_list[i], stack, length);
	  if (length%4 != 0) {
	    copyout(padding, stack + length + 1, length%4);
	  }
	  argvNew[argc - 1 - i] = stack;
	}
	
	for(i = stack; i < tempstack; i++){
	  kprintf("%c\n", *i);
	}*/
